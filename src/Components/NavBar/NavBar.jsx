...
function NavBar({title,iconClass,imgSrc,navButtonList}){
    ...
    const scrollTo=(elementName,link)=>{
        if(link==='')
        {
    ...
    const sideNavScrollTo=(elementName,link)=>{
        if(link==='')
        {
            const element = document.getElementById(elementName);
            element.scrollIntoView({block: "end"});
                ...
                <div className="nav-bar-side-container">
                    <ThemeButton/>
                    {
                        navButtonList.map((navButton,index)=>
                            <NavButton navButtonClassName="nav-button-holder" key={index} {...navButton} activePath={location.pathname} handleEvent={scrollTo} />
                        )
                    }
